...
basic_plt<- ggplot(mapping = aes(x = obs)) + geom_histogram(mapping = 
  ...
  geom_line(mapping = aes(x = xs, y = gb(xs), color = 'Proposal Background'),
            lwd = 1.3) + 
  # geom_line(mapping = aes(x = xs, y = fs(xs), color = 'Signal'),
  #           lwd = 1.3) +
  # geom_line(mapping = aes(x = den$x, y = den$y, color = 'kernel-est'),
  #           lwd = 1.3) +
...
for (j in 1:length(T_basis)) {
  fun_vec <- sapply(obs_Gb, T_basis[[j]])
...
fitted_plot <- ggplot(mapping = aes(x = obs)) + geom_histogram(mapping = 
                                                                 aes(y = after_stat(density)),
                                                               fill = 'steelblue', col = 'black',
                                                               breaks = hs$breaks) + 
  geom_line(mapping = aes(x = xs, y = y_real, color = 'Actual Mixture'),
