...
BW_epoly_likelihood <- function(rate)
{
  total_mass <- integrate(f = function(x){
    1/((x-bkg_loc)^2 + bkg_scale^2) + exp(-rate*x)
  }, l, u)$value
  
  fi <- sapply(obs, function(x) {
    (1/((x-bkg_loc)^2 + bkg_scale^2) + exp(-rate*x))/total_mass
  })
  return(-sum(log(fi)))
}
(qb_rate <- nlminb(start = 0.01,
                  objective = BW_epoly_likelihood,
                  lower = 0.005, upper = 0.33)$par) # tried with upper = Inf, 
                                                    # qb_rate came out close to 1
                                                    # but even then eploy_mass was
                                                    # of order 10^-49 so decided to 
                                                    # keep upper = 0.33 for ease of
                                                    # simulating data from epoly component

qb_ <- function(x) 1/((x-bkg_loc)^2 + bkg_scale^2) + exp(-qb_rate*x)
qb_mass <- integrate(qb_, l, u)$value
epoly_mass <- integrate(function(x) exp(-qb_rate*x), l, u)$value
# BW_mass <- integrate(function(x) 1/((x-bkg_loc)^2 + bkg_scale^2), l, u)$value
qb <- function(x) (1/((x-bkg_loc)^2 + bkg_scale^2) + exp(-qb_rate*x))/qb_mass

obs_from_epoly <- rtrunc(1e4, spec = 'exp',
                         a = l, b = u,
                         rate = qb_rate)
obs_from_BW <- rtrunc(1e4, spec = 'cauchy', a = l, b = u,
                      location = bkg_loc, scale = bkg_scale)
bkg_only_data <- ifelse(rbinom(1e4, size = 1, prob = epoly_mass/qb_mass),
                        obs_from_epoly, obs_from_BW)

hist(obs, breaks = 50, probability = TRUE)
curve(dtrunc(x, spec = 'cauchy', a = l, b = u,
             location = bkg_loc, scale = bkg_scale), 
      l, u, add = TRUE, lwd = 2, col = 'red', lty = 1)
curve(dtrunc(x, spec = 'exp', a = l, b = u,
             rate = qb_rate), 
...
sf_bkg_likelihood <- function(eps, mu, data)
{
  fbi <- sapply(data, function(x){
    eps*dtrunc(x, spec = 'norm', a = l, b = u,
               mean = mu, sd = sd_sig) + 
      (1-eps)*((1/((x-bkg_loc)^2 + bkg_scale^2) + (exp(-qb_rate*x)))/qb_mass)
  })
