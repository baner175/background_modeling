...
search_signal <- function(lambda, boot = 1e5, seed = 12345)
{
  ...
  for(i in 1:boot)
  {
    ni_B <- sapply(ni, function(x) rpois(1, x))
    theta_check_B <- mean(S1_vec*ni_B)
    t_stat_theta_B[i] <- k*theta_check_B/sqrt(sum(ni_B*S1_vec^2))
  }
  
  S_hat <- N_mid*eta_hat_binned
  B_hat <- N_mid*(1-eta_hat_binned)
  signif <- S_hat/sqrt(B_hat)
  ci_lower <- quantile(t_stat_theta_B, 0.05)
  # 
  # 
  ...
  return(c(eta_hat_binned, ci_lower, round(S_hat, 2), round(signif, 3)))
}
...
legend('topright', col = c('black', mycols),
       ...
       legend=c('KDE',
                TeX(sprintf(r'($\lambda = %f (\hat{\eta}_{(b)}: %f)$)', lambda_seq,
                            round(as.numeric(res_sig_search[,2]), 4)))),
       cex = 1,
       y.intersp = 1)

title(main = 'VBF - Cat 1')
