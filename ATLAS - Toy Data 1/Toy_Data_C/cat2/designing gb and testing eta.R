...
#signal parameters:
mean_sig <- 124.74; sd_sig <- 2.75
aL <- 1.39; aR <- 1.58;
nL <- 5.59; nR <- 9.13
eps <- 5*1e-2


dat <- read.csv('../Data/pseudo_unbinned_cat2.csv', header = TRUE)
obs <- dat$x
n <- length(obs)
...
mean1_in_gb <- (M_lower + mean_sig)/2-1; sd_in_gb <- 3*sd_sig
mean2_in_gb <- (M_upper + mean_sig)/2+1;
...
curve(kde, M_lower-2, M_upper+2, xlab = 'y',
