...
dat <- read.csv('../Data/pseudo_unbinned_cat3.csv', header = TRUE)
obs <- dat$x
n <- length(obs)
...
mean1_in_gb <- (M_lower + mean_sig)/2-2; sd_in_gb <- 5*sd_sig
mean2_in_gb <- (M_upper + mean_sig)/2+2;
...
legend(x = 140, y = 0.04, col = 1:length(fs_prop_seq),
