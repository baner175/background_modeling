...
u <- 160; l <- 110
n_breaks <- 50
obs <- read.csv('../Data/toydata_sig100_bkg5000.csv', header = TRUE)$x

hist(obs, probability = TRUE, breaks = n_breaks)

n <- length(obs)
obs_normalised <- scale(obs)
kde_ <- kdensity(obs_normalised, bw = 0.08)
...
mod_spur <- function(pars, data)
{
  alpha <- pars[1]
  bkg_loc <- pars[2]; bkg_scale <- pars[3]
  fs <- dtrunc(data, spec = 'norm', a = l, b = u, 
               mean = mean_sig, sd = sd_sig)
  qb <- dtrunc(data, a = l, b = u, spec = 'cauchy',
               location = bkg_loc, scale = bkg_scale)
  f_spur <- alpha*fs+(1-alpha)*qb
  return(-sum(log(f_spur)))
}
...
res <- constrOptim(theta = c(mean((obs<M_upper)&(obs>M_lower)),
                             l-5, 2*sd_sig),
                   f = mod_spur, data = obs,
                   ui = ui, ci = ci, grad = NULL)

(bkg_loc <- res$par[2]) # 93.80012
(bkg_scale <- res$par[3]) # 7.144723
