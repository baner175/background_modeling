...
u <- 160; l <- 110
n_breaks = 50
obs <- read.csv('../Data/toydata_sig50_bkg3333.csv', header = TRUE)$x
...
(bkg_loc <- res$par[2]) # 94.18189
(bkg_scale <- res$par[3]) # 7.168029
...
(lam_hat <- max(res_list)) # 0.0111258
...
res_safeguard <- nlminb(start = 0.1,
                        ...
                        upper = 1,
                        dat = obs)
(eta_safeguard <- res_safeguard$par) # 0.01103053
