...
option_list <- list(
  ...
  make_option(c('--sig_fact'), type = "double", default = 1,
              help = 'sigma factor in fs_gb', metavar = "number"),
  make_option(c('--fs_mix'), type = "double", default = 0.5,
              help = 'mixture component for fs in gb: unif_fs', metavar = "number"),
...
fs_mix <- as.numeric(opt$fs_mix); sig_fact <- as.numeric(opt$sig_fact)


# simulation results with known beta:

binned_res <- sapply(n_bins, function(k){
  simulated_power_binned_unif_fs(eta = eta, nbins = k, T_phys = n_phys,
                                 r = bkg_to_phys_ratio, nsims = B,
                                 seed = 12345,
                                 fs_mix = fs_mix,
                                 fs_gb_sig_factor = sig_fact,
                                 signif.level = 0.05)
})


unbinned_res <- simulated_power_unbinned_unif_fs(eta = eta, n_phys = n_phys,
                                                 r = bkg_to_phys_ratio, nsims = B,
                                                 seed = 12345, 
                                                 fs_mix = fs_mix,
                                                 fs_gb_sig_factor = sig_fact,
