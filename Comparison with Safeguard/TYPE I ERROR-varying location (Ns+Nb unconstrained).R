...
mix_mod_sf <- function(pars, data)
{
  Ns <- pars[1]; Nb <- pars[2]
  N <- length(data)
  pois <- dpois(x = N, lambda = Ns+Nb, log = TRUE)
  eta <- Ns/(Ns+Nb)
...
for(j in 1:B)
{
  ...
  sf_sim <- nlminb(start = c(1, length(samp_sim)-1),
  ...
  S1_vec <- (fs(samp_sim)/gb_test(samp_sim, fs_prop = 0.01) - 1)/norm_S_3
  theta <- mean(S1_vec)
  se_theta <- sqrt((mean(S1_vec^2) - theta^2)/n_sim)
  ...
  p_vals <- rbind(p_vals, c(p_val_sf, p_val_1, p_val_2, p_val_3))
  test_stat <- rbind(test_stat, c(test_stat_sf, t_stat_1, t_stat_2, t_stat_3))
  eta_hat <- rbind(eta_hat, c(eta_hat_sf, eta_hat_1, eta_hat_2, eta_hat_3))
...
colnames(p_vals) <- colnames(eta_hat) <- colnames(test_stat) <- 
  c('Safeguard', 'fs_prop=0.002', 'fs_prop=0.005', 'fs_prop=0.01')

# write.csv(p_vals,
#           file = 'Simulation Results/Type 1 error p-values.csv',
#           row.names = FALSE)
# 
# write.csv(eta_hat,
#           file = 'Simulation Results/Type 1 error eta_hat.csv',
#           row.names = FALSE)
# 
# write.csv(test_stat,
#           file = 'Simulation Results/Type 1 error test_stat.csv',
#           row.names = FALSE)


p_vals <- read.csv('Simulation Results/Type 1 error p-values.csv', header = TRUE)
eta_hat <- read.csv('Simulation Results/Type 1 error eta_hat.csv', header = TRUE)
test_stat <- read.csv('Simulation Results/Type 1 error test_stat.csv', header = TRUE)

# Type 1 error for safeguard:
mean(p_vals$Safeguard<=0.05) # 0.2417 # corresponding delta: 0.01340076

# checking delta:
sapply(c(0.002, 0.005, 0.01), function(lam){
