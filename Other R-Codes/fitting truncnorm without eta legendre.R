rm(list = ls())
library(ggplot2)
source('functions truncnorm without eta legendre.R')
# data generation
set.seed(12345)
n <- 2e3
...
plt_1_Leg <- ggplot(mapping = aes(x = obs)) + geom_histogram(mapping = 
                                                           aes(y = after_stat(density)),
                                                         fill = 'steelblue', col = 'black',
                                                         breaks = hs$breaks) + 
  geom_line(mapping = aes(x = xs, y = y_real, color = 'Actual Mixture'),
...
plt_2_Leg <- ggplot(mapping = aes(x = obs)) + geom_histogram(mapping = 
                                                               aes(y = after_stat(density)),
                                                             fill = 'steelblue', col = 'black',
                                                             breaks = hs$breaks) + 
  geom_line(mapping = aes(x = xs, y = y_real, color = 'Actual Mixture'),
...
plt_3_Leg <- ggplot(mapping = aes(x = obs)) + geom_histogram(mapping = 
                                                               aes(y = after_stat(density)),
                                                             fill = 'steelblue', col = 'black',
                                                             breaks = hs$breaks) + 
  geom_line(mapping = aes(x = xs, y = y_real, color = 'Actual Mixture'),
...
final_plt_Leg <- ggpubr::ggarrange(plt_1_Leg, plt_2_Leg, plt_3_Leg,
                               ncol = 3, nrow = 1,
                               common.legend = TRUE, legend = 'bottom')
final_plt_Leg

save.image(file = "fitting truncnorm without eta legendre.RData")
