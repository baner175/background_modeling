...
beta_sp_hat <- fit0$par[2]
beta_sp_hat_se <- sqrt(diag(solve(hess))[2])/sqrt(m)

eps_sp_hat <- fit0$par[1]
eps_sp_hat_se <- sqrt(diag(solve(hess))[1])/sqrt(m)
...
spurious_joint_likelihood_C <- function(par){
  ...
  beta_log_lik <- dlnorm(beta, meanlog = log(beta_sp_hat), 
                         sdlog = beta_sp_hat_se/abs(beta_sp_hat), 
                         log = TRUE)
  # eps_log_lik <- dnorm(eps, mean = eps_sp_hat,
  #                       sd = eps_sp_hat_se,
  #                       log = TRUE)
  eps_log_lik <- dlnorm(eps, meanlog = log(eps_sp_hat),
                        sdlog = eps_sp_hat_se/abs(eps_sp_hat),
                        log = TRUE)
...
spurious_phys_res <- nloptr(x0 = c(0.01, eps_sp_hat, beta_sp_hat),
...
spurious_null_res <- nloptr(x0 = c(eps_sp_hat, beta_sp_hat),
                            eval_f = spurious_joint_likelihood_null,
                            opts = opts,
                            lb = c(0, -Inf),
                            ub = c(1, Inf))

eps_sp_phys_hat <- spurious_phys_res$solution[2]
beta_sp_phys_hat <- spurious_phys_res$solution[3]
eta_sp_hat <- spurious_phys_res$solution[1]
...
sp_row <- data.frame(
  'Regular spur with Constraint',
  eta_sp_hat, 
  p_val_sp, 
  max(qnorm(p_val_sp, lower.tail = FALSE) |> round(1), 0),
  max(round(eta_sp_hat*n, 2), 0))


qb_sp_ <- function(x) (x^(beta_sp_phys_hat))/((x-bkg_loc)^2 + bkg_scale^2)
qb_sp_mass <- integrate(qb_sp_, l, u)$value
qb_sp <- function(x) (x<=u)*(x>=l)*((x^(beta_sp_phys_hat))/((x-bkg_loc)^2 + bkg_scale^2))/qb_sp_mass
qb_sp <- Vectorize(qb_sp)
gb_sp <- function(x){
  fs <- dtrunc(x, spec = 'norm', a = l, b = u,
               mean = mean_sig, sd = sd_sig)
  qb <- (x<=u)*(x>=l)*((x^(beta_sp_phys_hat))/((x-bkg_loc)^2 + bkg_scale^2))/qb_sp_mass
  
  return(eps_sp_phys_hat*fs + (1-eps_sp_phys_hat)*qb)
}
...
# Here we shall proceed with the estimated beta in qb for only mu = 125, 
...
spurious_max_phys_res <- nloptr(x0 = c(0.01, eps_sp_max_hat, beta_sp_hat),
...
spurious_max_null_res <- nloptr(x0 = c(eps_sp_max_hat, beta_sp_hat),
...
colnames(res_sig_search) <- colnames(sp_row) <- colnames(sp_max_row) <- colnames(mle_row) <- colnames(unbiased_row) <- 
...
hist(bkg_data, probability = TRUE, breaks = 50,
     col = 'white', 
     main = TeX('Densities $g^{spur}_b(x;\\hat{\\epsilon},\\hat{\\beta})$, $g^{spur-max}_b(x;\\hat{\\epsilon},\\hat{\\beta})$ and $q_b^{MLE}(x; \\hat{beta}_{bkg})$ - on bkg'))
curve(gb_sp, l, u, add = TRUE,
      col = ggplot2::alpha('blue', 0.6), lwd = 2, lty = 3)
...
legend('topright',
       ...
       lwd = 2.2,
       legend = c(
         TeX(sprintf(r'($g^{spur}_b(x; \hat{\epsilon} = %.4f, \hat{\beta})$)', eps_sp_hat)),
