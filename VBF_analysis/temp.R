...
sp_beta_eps_likelihood <- function(par)
{
  eps <- par[1]; beta <- par[2]
...
beta_hat_sp <-  nlminb(start = c(0.01, 0.01),
                       objective = sp_beta_eps_likelihood,
                       lower = c(0, -Inf),
                       upper = c(1, Inf))$par[2]

sp_eps_likelihood <- function(par, mu)
{
  eps <- par
  
  qb_mass <- integrate(function(t){
    (t^beta_hat_sp)/((t-bkg_loc)^2 + bkg_scale^2)
  }, l, u)$value
  
  qb_i <- sapply(bkg_data, function(t){
    ((t^beta_hat_sp)/((t-bkg_loc)^2 + bkg_scale^2))/qb_mass
  })
  fs_i <- dtrunc(bkg_data, spec = 'norm', a = l, b = u, 
                 mean = mu, sd = sd_sig)
  fi <- (1-eps)*qb_i + eps*fs_i
...
{
  nlminb(start = 0.01,
         objective = sp_eps_likelihood,
         lower = 0, upper = 1,
         mu = t)$par
})

(gb_sp_eps <- max(sp_res))
...
curve(qb_sp, col = 'blue', lwd = 2, lty = 1, add = TRUE)
curve(gb_sp, col = 'red', lwd = 2, lty = 2, add = TRUE)

# norm_S_gb_sf <- integrate(function(t) ((fs(t)/gb_sf(t) - 1)^2)*gb_sf(t),
#                           l, u)$value |> sqrt()

sp_phys_likelihood <- function(eta)
{
  fs <- dtrunc(phys_data, spec = 'norm', a = l, b = u,
               mean = mean_sig, sd = sd_sig)
  qb <- sapply(phys_data, function(x){
    (x<=u)*(x>=l)*((x^(beta_hat_sp))/((x-bkg_loc)^2 + bkg_scale^2))/qb_sp_mass
  })
  gb <- gb_sp_eps*fs + (1-gb_sp_eps)*qb
  fi <- eta*fs+(1-eta)*gb
  return(-sum(log(fi)))
}

sp_signal_search_res <- nlminb(start = 0,
                               objective = sp_phys_likelihood,
                               lower = 0, upper = 1)

# estimated proportion from safeguard:
(eta_sp <- sp_signal_search_res$par)

ll0 <- -sp_phys_likelihood(eta = 0)
ll1 <- -sp_signal_search_res$objective

# safeguard p-value:
(p_val_sp <- 0.5*pchisq(-2*(ll0-ll1),
                        df = 1, lower.tail = FALSE))

sp_row <- data.frame(
  'safeguard', eta_sp, p_val_sp, qnorm(p_val_sp, lower.tail = FALSE) |> round(1),
  round(eta_sp*n, 2))
################################################################################
