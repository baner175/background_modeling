...
sf_bkg_likelihood <- function(par, mu, bin_ends, bin_counts)
{
  eps <- par[1]; rate <- par[2]
  qb_mass <- integrate(function(t){
    (exp(rate*t))/((t-bkg_loc)^2 + bkg_scale^2)
  }, l, u)$value
  ...
  for(i in 1:(length(bi)-1))
  {
    mu_i[i] <- N_total * integrate(f = function(x){
      eps*dtrunc(x, spec = 'norm', a = l, b = u,
                 mean = mu, sd = sd_sig) + 
        (1-eps)*(((exp(rate*x))/((x-bkg_loc)^2 + bkg_scale^2))/qb_mass)
...
{
  nlminb(start = c(0.01, 0.01),
         objective = sf_bkg_likelihood,
         lower = c(0, -Inf), upper = c(1, Inf),
         mu = t, bin_ends = bins, bin_counts = ni_bkg)$par
})

(gb_sf_eps <- sf_res[,which.max(sf_res[1,])][1])
(qb_rate <- sf_res[2,mu_seq == mean_sig])

# qb used for safeguard
qb_ <- function(x) (exp(qb_rate*x))/((x-bkg_loc)^2 + bkg_scale^2)
qb_mass <- integrate(qb_, l, u)$value
qb <- function(x) ((exp(qb_rate*x))/((x-bkg_loc)^2 + bkg_scale^2))/qb_mass
    ...
    mu_i[i] <- N_total * integrate(f = function(x){
      eta*dtrunc(x, spec = 'norm', a = l, b = u,
        ...
        (1-eta)*(gb_sf_eps*dtrunc(x, spec = 'norm', a = l, b = u,
                                  mean = mean_sig, sd = sd_sig) + 
                   (1-gb_sf_eps)*(((exp(qb_rate*x))/((x-bkg_loc)^2 + bkg_scale^2))/qb_mass))
    }, lower = bi[i], upper = bi[i+1])$value
  }
  return(sum(mu_i - log(mu_i)*bin_counts))
}
...
caption <- paste0("Signal Search Results: ", 
                  'Cat',cat,'_',scenario,
                  '_mu',mu_part_1,'p',mu_part_2, '<br> Bkg: BW X Epoly')
